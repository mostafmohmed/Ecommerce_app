<?php

namespace App\services\front;

use Illuminate\Support\Facades\Http;

class Myyfatoorahservices
{
    /**
     * Create a new class instance.
     */
    // https://apitest.myfatoorah.com/v2/SendPayment
     public $baseurl;
    public $header;
 
    public function __construct()
    {
            $this->baseurl = env('Base_url');
            $this->header=['Authorization'=>'Bearer rLtt6JWvbUHDDhsZnfpAhpYk4dxYDQkbcPTyGaKp2TYqQgG7FGZ5Th_WD53Oq8Ebz6A53njUoo1w3pjU1D4vs_ZMqFiz_j0urb_BH9Oq9VZoKFoJEDAbRZepGcQanImyYrry7Kt6MnMdgfG5jn4HngWoRdKduNNyP4kzcp3mRv7x00ahkm9LAK7ZRieg7k1PDAnBIOG3EyVSJ5kK4WLMvYr7sCwHbHcu4A5WwelxYK0GMJy37bNAarSJDFQsJ2ZvJjvMDmfWwDVFEVe_5tOomfVNt6bOg9mexbGjMrnHBnKnZR1vQbBtQieDlQepzTZMuQrSuKn-t5XZM7V6fCW7oP-uXGX-sMOajeX65JOf6XVpk29DP6ro8WTAflCDANC193yof8-f5_EYY-3hXhJj7RBXmizDpneEQDSaSz5sFk0sV5qPcARJ9zGG73vuGFyenjPPmtDtXtpx35A-BVcOSBYVIWe9kndG3nclfefjKEuZ3m4jL9Gg1h2JBvmXSMYiZtp9MR5I6pvbvylU_PP5xJFSjVTIz7IQSjcVGO41npnwIxRXNRxFOdIUHn0tjQ-7LwvEcTXyPsHXcMD8WtgBh-wxR8aKX7WPSsT1O8d8reb2aR7K3rkV3K82K_0OgawImEpwSvp9MNKynEAJQS6ZHe_J_l77652xwPNxMRTMASk1ZsJL'];

    }
    public function getpaymentStatus($date){
       return $this->createRequest('v2/getpaymentstatus','POST',$date);
 
    }
    public function createRequest( $url,$method,$body=[]){
        if (empty($body)) {
          return false;
        }
        
        $response=Http::withHeaders($this->header)->
    timeout(10)->withoutVerifying()->send($method,$this->baseurl.$url,[
        'json'=> $body
        ]);
   if (!$response->successful()) {
       return false;
    } 
    return $response->json();
       
    }
    public function checkout($date){
return $this->createRequest('v2/SendPayment','POST',$date);
    }
}

...
if (pdSize) {
  ...
  pdOption.forEach((option) => {
    option.addEventListener("click", () => {
      let textElement = option.querySelector(".option-text");
      let measureElement = option.querySelector(".option-measure");

      if (textElement && measureElement) {
        pdBtn_Text.innerHTML = textElement.innerText;
        pdBtn_Text2.innerHTML = measureElement.innerText;
        pdSize.classList.remove("active");
      } else {
        console.warn("بعض العناصر غير موجودة داخل هذا الاختيار");
      }
    });
  });
}

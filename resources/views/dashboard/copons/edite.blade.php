<div class="modal fade" id="editCouponModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      ...
      <form action="" id="updateecoupon" class="form" method="POST">
        @csrf  
        @method('PUT')
        <input  id="copon_id" type="hidden" name="id">
          <div class="form-group">
            <label for="name">{{ __('dashboard.code') }}</label>
            <input type="text" name="code" id="copon-code" class="form-control" id="copon-name"
                placeholder="{{ __('dashboard.code') }}">
                <p></p>
        </div>
        <div class="form-group">
            <label for="name">{{ __('dashboard.discount_perecentage') }}</label>
            <input type="text" name="discount_perecentage" class="form-control" id="copon-discount_perecentage"
                placeholder="{{ __('dashboard.discount_perecentage') }}">
                <p></p>
        </div>
        <div class="form-group">
            <label for="limit">{{ __('dashboard.limitation') }}</label>
            <input type="number" id="copon-limit" name="limit" class="form-control" id="single-image"
                placeholder="{{ __('dashboard.limitation') }}">
                <p></p>
        </div>
        <div class="form-group">
            <label for="limit">{{ __('dashboard.start_date') }}</label>
            <input type="date" name="start_data" class="form-control" id="copon-start_data"
                placeholder="{{ __('dashboard.start_date') }}">
                <p></p>
        </div>
        <div class="form-group">
            <label for="limit">{{ __('dashboard.end_date') }}</label>
            <input type="date" name="end_data" class="form-control" id="copon-end_data"

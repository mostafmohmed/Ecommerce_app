...
@foreach ($orderusers->orderitem as $item)
...
<span class="number">{{$item->produect_quantity}}</span>
<span class="plus">
+
</span>
</div>
...
<div class="table-wrapper-center">
    ...
    $sum=0
        foreach ($item as $key => $value) {
           $sum= $value->product->price*$value->produect_quantity+$sum; # code...
        }
    @endphp --}}
   
<h5 class="heading"> {{$item->produect_quantity * $item->product->price  }}</h5>
</div>
</td>
<td class="table-wrapper">
<div class="table-wrapper-center">
